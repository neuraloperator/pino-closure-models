...
class Trainer:
    ...
    def train(self, train_loaders, test_loaders,
        ...
        """
        #add here
        use_loader=2 # loader in the outer loop
        use_loader_128=0 # loader in the inner loop
        use_loader_1024=2
            ...
            for idx, sample in enumerate(train_loader):###
                ...
                if self.overrides_loss:
                    ...
                    if self.amp_autocast:
                        with amp.autocast(enabled=True):
                            if isinstance(out, torch.Tensor):
                                loss = training_loss[0](out.float(), **sample,t_val=train_loaders[use_loader][1])#[1]: value of t_val, time period per real data
                            elif isinstance(out, dict):
                                loss += training_loss[0](**out, **sample,t_val=train_loaders[use_loader][1])
                    else:
                        if isinstance(out, torch.Tensor):
                            loss = training_loss[0](out.float(), **sample,t_val=train_loaders[use_loader][1])
                        elif isinstance(out, dict):
                            loss += training_loss[0](**out, **sample,t_val=train_loaders[use_loader][1])

                del out, sample['x'],sample['y'] ## if not, these sample tensors will be kept on GPU and cause a memory blowup
                ...
                if K128:  #The number of cgs data batches together with one frs dara batch
                    ...
                    for sample in sample2:
                        ...
                        if self.amp_autocast:
                            ...
                            out = self.model(**sample)  ###
                        lossq=0 # Supervised loss of cgs data.
                        ...
                        if self.overrides_loss:
                            ...
                            if self.amp_autocast:
                                with amp.autocast(enabled=True):
                                    if isinstance(out, torch.Tensor):
                                        lossq= training_loss[1](out.float(), **sample, t_val=train_loaders[use_loader][
                                            1])  # [1]: value of t_val, time period per real data
                                    elif isinstance(out, dict):
                                        loss += training_loss[1](**out, **sample, t_val=train_loaders[use_loader][1])
                            else:
                                if isinstance(out, torch.Tensor):
                                    lossq= training_loss[1](out.float(), **sample, t_val=train_loaders[use_loader][1])
                                elif isinstance(out, dict):
                                    loss += training_loss[1](**out, **sample, t_val=train_loaders[use_loader][1])
