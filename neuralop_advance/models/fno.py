...
class FNO(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """New:
        check_mem: if check_mem, print cuda memory after each layer
        mlp_splt: int or list(require len=3). Number of partitions for lifting, hidden(in fno block), projecting layer.
                    Split the third dim and use for loop. Dim of input tensor doesn't have to be divisible by mlp_splt.
        fft_splt: int. Split in_channel(second dim) in FNO block during fft.
                        Dim of input tensor doesn't have to be divisible by mlp_splt.
        grad_chkpt: bool or list of 5 bools, use gradient checkpoint or not.
                    [fno_layers, lifting mlp, fno block mlp, proj mlp, fft channel]
        """


        self.check_mem=kwargs['check_mem'] if 'check_mem' in kwargs else None
        ...
        if self.chkpt:
            self.ly_dvc=kwargs['layer_dvc']
            self.ly_dvc=[str_to_dvc(x)for x in self.ly_dvc]
            self.ly_splt=kwargs['layer_split'] if isinstance(kwargs['layer_split'],list) else [kwargs['layer_split']]*2
            self.blk_splt_fft=kwargs['block_fft_split'] if isinstance(kwargs['block_fft_split'],list) else [kwargs['block_fft_split']]*self.n_layers
            self.blk_splt_mlp=kwargs['block_mlp_split'] if isinstance(kwargs['block_mlp_split'],list) else [kwargs['block_mlp_split']]*self.n_layers
            self.blk_dvc=kwargs['block_dvc']
            self.blk_dvc_y3=kwargs['block_dvc_y3']
            self.blk_dvc_y0=kwargs['block_dvc_y0']
            self.save_ls=[] # intermediates after lifting, block,1,2,3,4, proj (i.e. output)
        ...
        if not self.chkpt:
            ...
            self.fno_blocks = FNOBlocks(
                ...
                blk_splt_fft=self.blk_splt_fft,
                blk_splt_mlp=self.blk_splt_mlp,
                blk_dvc=self.blk_dvc,
                blk_dvc_y3=self.blk_dvc_y3,
                blk_dvc_y0=self.blk_dvc_y0,
                check_mem=self.check_mem,
                check_mem_bp=self.check_mem,
                # **kwargs
            )
        ...
        if self.chkpt:
            self.lifting=sinc(self.lifting,chkpt=1,splt=self.ly_splt[0],dim=2,dvc_chkpt=self.ly_dvc[0],dvc_cpt=self.dvc_cpt,no_splt=self.no_splt)
        else:
            self.lifting=sinc(self.lifting,chkpt=0,no_splt=self.no_splt,dvc_cpt=self.dvc_cpt)
        ...
        if self.chkpt:
            self.projection=sinc(self.projection,chkpt=1,splt=self.ly_splt[1],dim=2,dvc_chkpt=self.ly_dvc[1],dvc_cpt=self.dvc_cpt,no_splt=self.no_splt)
        else:
            self.projection=sinc(self.projection,chkpt=0,no_splt=self.no_splt,dvc_cpt=self.dvc_cpt)

    def forward(self, x, output_shape=None, test=False,**kwargs):
        """TFNO's forward pass
    ...
    def bp(self,x,y_grad=None,keep_x_grad=True,dvc_grad_x=None,output_shape=None,**kwargs):
        """To do: check if anything need to be done regarding output_shape """
        if not self.chkpt:
            return
        temp = mmm('BP start') if self.check_mem else None
        if output_shape is None:
            output_shape = [None]*self.n_layers
        elif isinstance(output_shape, tuple):
            output_shape = [None]*(self.n_layers - 1) + [output_shape]

        self.save_ls[-1].requires_grad=False
        '''to test: save grad_temp on gpu?——Not good.(32 : 1)'''

        grad_temp=self.projection.bp(self.save_ls[-2],self.save_ls[-1].grad,dvc_grad_x=self.blk_dvc[-1])
        temp = mmm('BP proj layer') if self.check_mem else None

        del self.save_ls[-1]#output of proj
        del self.save_ls[-1]# outout of final block
        torch.cuda.empty_cache()
        temp = mmm('del 2 chkpts') if self.check_mem else None

        if self.domain_padding is not None:
            grad_temp=self.domain_padding.pad(grad_temp)
        temp = mmm('pad grad_temp') if self.check_mem else None
        #
        for layer_idx in range(self.n_layers-1,-1,-1):

            if layer_idx>0:
                self.fno_blocks.bp(self.save_ls[-1],y_grad=grad_temp,index=layer_idx,output_shape=output_shape[layer_idx])
                ...
                if self.domain_padding is not None:
                    ...
                    yy=self.domain_padding.pad(self.save_ls[-1])
                    del self.save_ls[-1]
                    self.fno_blocks.bp(yy, y_grad=grad_temp, index=layer_idx,output_shape=output_shape[layer_idx])

                    grad_temp = self.domain_padding.unpad(grad_temp)

                else:
                    self.fno_blocks.bp(self.save_ls[-1], y_grad=grad_temp, index=layer_idx,output_shape=output_shape[layer_idx])

                    del self.save_ls[-1]
            temp = mmm(f'BP layer={layer_idx}') if self.check_mem else None

        self.lifting.bp(x,grad_temp,keep_x_grad=False)
        temp = mmm('BP lift layer') if self.check_mem else None
        # if self.check_mem:
        #     exit()
        return
