...
class PositionalEmbedding():
    ...
    def grid(self, data):#data: 1*gx*gy  (one element of n*1*gx*gy) , will serve as "transform_x"function in train_db
        if self._grid is None:
            self._grid = get_grid_positional_encoding(data, 
                                                      grid_boundaries=self.grid_boundaries,
                                                      channel_dim=self.channel_dim) #1*1*gx*gy
        return self._grid
    ...
    def __call__(self, data):
        ...
        return torch.cat((data, x, y), dim=0)#3*gx*gy
