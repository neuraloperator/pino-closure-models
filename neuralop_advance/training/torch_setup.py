...
def setup(config):
    ...
    if config.distributed.use_distributed:
        ...
        assert isinstance(config.data.n_train,(list,int))
        if isinstance(config.data.n_train,int):
        ...
        elif isinstance(config.data.n_train,list):
            for j in range(len(config.data.n_train)):
                assert (config.data.n_train[j] % config.data.batch_size== 0), (
                    f'The number of training samples={config.data.n_train[j]}'
                    f' cannot be divided by the batch_size={config.data.test_batch_sizes}'
                    f' for test resolution {config.data.resolution[j]}.'
                )

        for j in range(len(config.data.n_tests)):
            assert(config.data.n_tests[j] % config.data.test_batch_sizes[j] == 0), (
                f'The number of testing samples={config.data.n_tests[j]}'
