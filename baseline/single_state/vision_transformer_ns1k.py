...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        torch._assert(image_size[-1] % patch_size[-1] == 0, "Input shape indivisible by patch size!")
        torch._assert(image_size[-2] % patch_size[-2] == 0, "Input shape indivisible by patch size!")
        ...
        seq_length = (image_size[-1] // patch_size[-1])* (image_size[-2] // patch_size[-2]) #** 2
        ...
        if representation_size is None:#this way
            heads_layers["head"] = nn.Linear(1728, num_classes)
        ...
        self.pool_before_head=nn.AvgPool1d(kernel_size=8)

    def _process_input(self, x: torch.Tensor) -> torch.Tensor:
        n, c, h, w = x.shape
        p = self.patch_size#[-1]
        torch._assert(h == self.image_size[0], f"Wrong image height! Expected {self.image_size[0]} but got {h}!")
        torch._assert(w == self.image_size[1], f"Wrong image width! Expected {self.image_size[1]} but got {w}!")
        n_h = h // p[0]
        n_w = w // p[1]
...
def _vision_transformer(
    ...
    **kwargs: Any,
    ...
    #image_size = kwargs.pop("image_size", 16)
...
def vit_b_kf(*, weights: Optional[ViT_B_32_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
    ...
    .. autoclass:: torchvision.models.ViT_B_32_Weights
        :members:
    """
    weights = None #ViT_B_32_Weights.verify(weights)

    return _vision_transformer(
        patch_size=[4, 4],
        num_layers=2,
        num_heads=6,
        hidden_dim=768//8, #768//4,
        mlp_dim=128, #3072//8,
        weights=weights,
        progress=progress,
        image_size=[16, 16],
...
def vit_b_ks(*, weights: Optional[ViT_B_32_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
    Constructs a vit_b_32 architecture from
    `An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale <https://arxiv.org/abs/2010.11929>`_.
