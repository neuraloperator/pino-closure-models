...
class Trainer:
    ...
    def train(self, train_loaders, test_loaders,
            optimizer, scheduler, regularizer,
              training_loss=None, eval_losses=None,losstype='sum',K128=0,lam128=0,epoch_pde=0,
              pde_rate_inc = 0,lam_of_pde=0,renew_opt=0,
              pde_inc_time = 1000):
        ...
        #   loader has changed inner structure
        if self.callbacks:
        ...
        for epoch in range(self.n_epochs):
                    ...
                    for sample in sample2:
                        ...
                        if self.overrides_loss:
                            ...
                            if self.amp_autocast:
                                with amp.autocast(enabled=True):
                                    if isinstance(out, torch.Tensor):
                                        lossq= training_loss[1][epoch>epoch_pde](out.float(), **sample,lam=lam_of_pde, t_val=train_loaders[use_loader][
                                            1])  # [1]: value of t_val, time period per real data
                                    elif isinstance(out, dict):
                                        loss += training_loss[1][epoch>epoch_pde](**out, **sample,lam=lam_of_pde, t_val=train_loaders[use_loader][1])
                            else:
                                if isinstance(out, torch.Tensor):
                                    lossq= training_loss[1][epoch>epoch_pde](out.float(), **sample,lam=lam_of_pde, t_val=train_loaders[use_loader][1])
                                elif isinstance(out, dict):
                                    loss += training_loss[1][epoch>epoch_pde](**out, **sample,lam=lam_of_pde, t_val=train_loaders[use_loader][1])
            ...
            if epoch>0 and epoch% pde_inc_time==0:
                lam_of_pde*=pde_rate_inc
                if renew_opt:
