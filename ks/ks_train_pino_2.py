...
pipe = ConfigPipeline(
    [
        YamlConfig(
            "./ks_pino_2.yaml", config_name="default", config_folder="../config"
        ),
...
#
cpt=torch.load(model_dict[128][config.wandb.model_use],map_location=device)
...
if config.wandb.savemd:
    print('start save')
    torch.save({'model':model.state_dict()},f'model_save/model_frs_{file_id}.pt')
    print('ok')
