...
from trainer_pino import Trainer
...
pipe = ConfigPipeline(
    [
        YamlConfig(
            "./ks_pino.yaml", config_name="default", config_folder="../config"
        ),
...
def gen_data_pad(x,dt,dim=1,dT=None,t0:int=0,t_end:int=None,single=0,rr=None):#x.shape=NTX ,dim(T)=1

    y=wcw.slicing_split(x,dim=dim,dt=dt,dT=dT,t0=t0,t_end=t_end,single=single) #shape=NKTX
    if rr==None:
...
if config.wandb.savemd:
    print('start save')
    torch.save({'model':model.state_dict()},f'model_save/model_cgs_{file_id}.pt')
    print('ok')
